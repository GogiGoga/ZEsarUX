...
UINT8 scmp_device_read_decrypted_byte(UINT16 a)
{
	return memory_data[a];
}
UINT8 scmp_device_read_raw_byte(UINT16 a)
{
	return memory_data[a];
}

UINT8 scmp_device_read_byte(UINT32 a)
{
	return memory_data[a];
}

void scmp_device_write_byte(UINT32 a, UINT8 v)
{
...
/*
scmp_device_scmp_device(const machine_config &mconfig, const char *tag, device_t *owner, UINT32 clock)
...
scmp_device_scmp_device(const machine_config &mconfig, device_type type, const char *name, const char *tag, device_t *owner, UINT32 clock, const char *shortname, const char *source)
...
ins8060_device_ins8060_device(const machine_config &mconfig, const char *tag, device_t *owner, UINT32 clock)
	: scmp_device(mconfig, INS8060, "INS 8060 SC/MP II", tag, owner, clock, "ins8060", __FILE__)
{
}

offs_t scmp_device_disasscmp_m_disassemble(char *buffer, offs_t pc, const UINT8 *oprom, const UINT8 *opram, UINT32 options)
{
...
UINT16 scmp_device_ADD12(UINT16 addr, INT8 val)
{
	return ((addr + val) & 0x0fff) | (addr & 0xf000);
}

UINT8 scmp_device_ROP()
{
	UINT16 pc = scmp_m_PC.w.l;
...
void scmp_device_BIN_ADD(UINT8 val)
{
	UINT16 tmp = scmp_m_AC + val + ((scmp_m_SR >> 7) & 1);
	UINT8 ov = (((scmp_m_AC & 0x80)==(val & 0x80)) && ((scmp_m_AC & 0x80)!=(tmp & 0x80))) ? 0x40 : 0x00;
...
void scmp_device_DEC_ADD(UINT8 val)
{
	UINT16 tmp = scmp_m_AC + val + ((scmp_m_SR >> 7) & 1);
	if ((tmp & 0x0f) > 9) tmp +=6;
	scmp_m_AC = tmp % 0xa0;
...
UINT16 scmp_device_GET_ADDR(UINT8 code)
{
	UINT16 addr = 0;
	INT8 offset = 0;
	UINT16 retVal = 0;
	UINT16 ptr = scmp_device_GET_PTR_REG(code & 0x03)->w.l;

	UINT8 arg = scmp_device_ARG();
	if (arg == 0x80) {
		offset = scmp_m_ER;
	} else {
		if (arg & 0x80) {
	...
	if (opcode&128) {
		...
		switch (opcode)
		{
			...
			case 0xa8 : case 0xa9 : case 0xaa : case 0xab :
						// IDL
						{
							UINT16 addr = scmp_device_GET_ADDR(opcode);
							scmp_m_icount -= 22;
			...
			case 0x8f:  // DLY
						tmp = scmp_device_ARG();
						scmp_m_icount -= 13 + (scmp_m_AC * 2) + (((UINT32)tmp) << 1) + (((UINT32)tmp) << 9);
						scmp_m_AC = 0xff;
						break;
			// Others are illegal
			default :   scmp_m_icount -= 1;
		...
		switch (opcode)
		{
			...
			case 0x3c:  case 0x3d :case 0x3e: case 0x3f:
						// XPPC
						{
							UINT16 tmp16 = scmp_device_ADD12(scmp_m_PC.w.l,-1); // Since PC is incremented we need to fix it
							scmp_m_icount -= 7;
...
{
	UINT16 tmp = scmp_device_ADD12(scmp_m_PC.w.l,-1); // We fix PC so at return it goes to current location
	scmp_m_SR &= 0xf7; // clear IE flag

	scmp_m_icount -= 8; // assumption
	// do XPPC 3
